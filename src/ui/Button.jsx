...
function Button({ children, disabled, className, to, type, onClick }) {
  const base =
  ...
  const style = {
    ...
    secondary:
      "inline-block text-sm rounded-full border-2 border-stone-300  font-semibold uppercase tracking-wide text-stone-400 transition-all duration-300 hover:bg-stone-300 hover:text-stone-800 focus:text-stone-800 focus:bg-stone-300 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed px-4 py-2.5 sm:px-6 sm:py-3.5 ",
  };

  if (to)
    return (
      <Link className={style[type] + " " + className} to={to}>
        {children}
      </Link>
    );
  ...
  return (
    <button disabled={disabled} className={style[type] + " " + className}>
      {children}
    </button>
  );
}
